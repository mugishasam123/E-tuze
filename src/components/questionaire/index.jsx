...
const QuestionaireComp = () => {
  ...
  return (
      <form>
        {
            questions.filter((question) => question.id === currentQuestion).map(item => (
                <>
                
                    
                    <label className='text-2xl font-bold text-center mb-10'>{item.question}</label>
                            <ul className='border rounded-md text-center'>
                                {
                                item.choice.map((choice, index) => (
                                    <>
                                    <option key={index} onClick={handleClick} value={choice} className="border-b py-4 cursor-pointer">{choice}</option>
